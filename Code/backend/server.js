...
app.post("/api/change-password", async (req, res) => {
  try {
    ...
    if (user.password_hash === old_password) {
      ...
      oldOk = await bcrypt.compare(old_password, user.password_hash);
    }
...
app.post("/api/admin/verify-password", async (req, res) => {
  try {
    ...
    res.json({
      success: ok,
      message: ok ? "Admin verified" : "Sai mat khau admin"
    });
  } catch (err) {
...
app.get("/api/users", async (req, res) => {
  try {
    const [rows] = await db.query("SELECT id, phone_number, username, role, is_active FROM users ORDER BY created_at DESC");
